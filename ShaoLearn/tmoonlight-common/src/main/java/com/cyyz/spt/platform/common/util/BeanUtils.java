...
public abstract class BeanUtils {
    ...
    /**
     * 字段copy, 将 abc 字段 拷贝到 abcSuffix 字段。
     * @param source
     * @param target
     * @param suffix
     * @throws BeansException
     */
    public static void copyPropertiesWithSuffix(Object source, Object target, String suffix) throws BeansException {
        copyPropertiesWithSuffix(source, target, null, suffix, (String[]) null);
    }
    /**
     ...
     * @throws BeansException
     */
    private static void copyPropertiesWithSuffix(Object source, Object target, Class<?> editable, String suffix,  String... ignoreProperties)
            throws BeansException {
        ...
        for (PropertyDescriptor targetPd : targetPds) {
            Method writeMethod = targetPd.getWriteMethod();
            if (writeMethod != null && (ignoreList == null || !ignoreList.contains(targetPd.getName())) && targetPd.getName().endsWith(suffix)) {
                PropertyDescriptor sourcePd = getPropertyDescriptor(source.getClass(), targetPd.getName().substring(0, targetPd.getName().lastIndexOf(suffix)));
                if (sourcePd != null) {
                    Method readMethod = sourcePd.getReadMethod();
                    if (readMethod != null &&
                            ClassUtils.isAssignable(writeMethod.getParameterTypes()[0], readMethod.getReturnType())) {
