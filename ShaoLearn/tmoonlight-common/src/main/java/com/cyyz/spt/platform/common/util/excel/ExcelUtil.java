...
import com.cyyz.spt.platform.common.util.Util;
...
public class ExcelUtil<T> {
    ...
    public static void createCell(HSSFRow row, Integer cellIndex, Object value){
        row.createCell(cellIndex++)
            .setCellValue(new HSSFRichTextString(Util.objToStr(value)));
    }

    //创建excel
    public static <T> HSSFWorkbook createExcel(ExcelProperties properties, List<T> dataList, ExcelCallback<T> callback){
        int rowIndex = 0;
        // 工作薄
        ...
        LinkedList<ExcelProperties.ColTitle> column = properties.getColumn();
        Integer columnWidth = 50 * 256;
        Integer columnIndex = 0;
        if (column != null){
            HSSFRow row = sheet.createRow(rowIndex++);
            for (ExcelProperties.ColTitle colTitle : column){
        ...
        contextStyle.setAlignment(HSSFCellStyle.ALIGN_LEFT);
